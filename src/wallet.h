...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 AWR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAWR send
enum ZerocoinSpendStatus {
    ZAWR_SPEND_OKAY = 0,                            // No error
    ZAWR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAWR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAWR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAWR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAWR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAWR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAWR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAWR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAWR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAWR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAWR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAWR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAWR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAWR_SPENT_USED_ZAWR = 14,                      // Coin has already been spend
    ZAWR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZAWR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZAWROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzAWRWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzAWRTracker> zpivTracker;
    ...
    void setZWallet(CzAWRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzAWRTracker>(new CzAWRTracker(strWalletFile));
    }

    CzAWRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zAWR reset */
    boost::signals2::signal<void()> NotifyzAWRReset;
