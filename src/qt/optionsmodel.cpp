...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AWR);
    ...
    if (!settings.contains("nAnonymizeAwardcoinAmount"))
        settings.setValue("nAnonymizeAwardcoinAmount", 1000);

    nAnonymizeAwardcoinAmount = settings.value("nAnonymizeAwardcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAwardcoinAmount"))
        SoftSetArg("-anonymizeawardcoinamount", settings.value("nAnonymizeAwardcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in awardcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAwardcoinAmount:
            return QVariant(nAnonymizeAwardcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAwardcoinAmount:
            nAnonymizeAwardcoinAmount = value.toInt();
            settings.setValue("nAnonymizeAwardcoinAmount", nAnonymizeAwardcoinAmount);
            emit anonymizeAwardcoinAmountChanged(nAnonymizeAwardcoinAmount);
            break;
        case CoinControlFeatures:
