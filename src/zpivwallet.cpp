...
CzAWRWallet::CzAWRWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAWRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAWRSeed_deprecated()) {
                    LogPrintf("%s: Updated zAWR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzAWRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAWRCount(nCountLastUsed);
    else if (!walletdb.ReadZAWRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAWRWallet::Lock()
{
    seedMaster = 0;
}

void CzAWRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAWRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAWR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAWRWallet::LoadMintPoolFromDB()
{
...
void CzAWRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAWRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAWRWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzAWRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAWR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzAWRWallet::SeedToZAWR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAWRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAWRWallet::GenerateDeterministicZAWR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAWRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAWRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
