...
public:
    ...
    bool WriteZAWRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZAWRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZAWRSeed_deprecated(uint256& seed);
    bool EraseZAWRSeed();
    bool EraseZAWRSeed_deprecated();

    bool WriteZAWRCount(const uint32_t& nCount);
    bool ReadZAWRCount(uint32_t& nCount);
